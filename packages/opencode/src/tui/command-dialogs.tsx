 ...
 */
export async function collectDialogFlow(
  command: CommandModalName,
  argumentsText: string,
  context: {
    settings: { get(): { log_level: string } }
  },
): Promise<DialogFlow> {
  switch (command) {
    case 'antigravity-quota':
      return {
        ...
        payload: {
          ...
          knobs: { mode: argumentsText === 'refresh' ? 'refresh' : 'status' },
        },
      }
      ...
      return {
        ...
        actions: [
          ...
          },
          {
            ...
            description:
              'Fall back to the alternate quota style on exhaustion.',
          },
        ],
        cancelLabel: CANCEL_LABEL,
        payload: {
    ...
    case 'antigravity-dump':
      return {
        ...
        payload: {
          ...
          knobs: { mode: argumentsText === 'on' ? 'enable' : 'status' },
        },
      }
    case 'antigravity-logging':
      return {
        ...
        actions: [
          { key: 'error', label: 'Error', description: 'Errors only.' },
          { key: 'warn', label: 'Warn', description: 'Warnings and errors.' },
          {
            key: 'info',
            label: 'Info',
 ...
 */
export interface RenderDialogFlowOptions {
  api: {
    ui: {
  ...
  flow: DialogFlow
  /**
   * Apply handler — receives the selected action's key (and the raw
   * arguments the dialog collected) and returns an `ApplyResult`.
   */
  apply: (command: CommandModalName, args: string) => Promise<{ text: string }>
  onClose?: () => void
}

export function renderDialogFlow(options: RenderDialogFlowOptions): void {
  const { api, flow, apply, onClose } = options
  const render = () => (
    <api.ui.DialogSelect
      title={flow.title}
      options={flow.actions.map((action) => ({
        title: action.label,
        value: action.key,
      ...
      onSelect={(selected) => {
        ...
        void apply(flow.command, selected.value as string).catch(() => {
