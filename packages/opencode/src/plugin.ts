...
} from './plugin/ui/quota-status'
import {
  getAntigravityVersionResolution,
          ...
          if (!isOAuthAuth(auth)) {
            ...
            if (drift.status === 'restorable' && drift.account) {
              ...
              try {
                ...
                log.info(
                  'Restored Antigravity OAuth auth from account storage',
                  {
                    reason: drift.reason,
