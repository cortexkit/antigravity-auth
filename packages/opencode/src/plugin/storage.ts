...
async function getLockFunction(): Promise<LockFunction> {
  ...
  const fn =
    (typeof mod.lock === 'function' ? mod.lock : undefined) ||
    (mod.default &&
    typeof (mod.default as Record<string, unknown>).lock === 'function'
      ? (mod.default as Record<string, unknown>).lock
      : undefined)
  if (typeof fn !== 'function') {
    ...
    throw new Error(
      'proper-lockfile did not expose a lock() function; cannot acquire account file lock',
    )
  }
...
const lockfile = {
  lock: (path: string, options?: Record<string, unknown>) =>
    getLockFunction().then((fn) => fn(path, options)),
}
    ...
    if (acc.refreshToken) {
      ...
      if (existingAcc) {
        ...
        const merged: AccountMetadataV3 = {
          ...
          managedProjectId:
            acc.managedProjectId ?? existingAcc.managedProjectId,
          rateLimitResetTimes: {
          ...
          eligibilityStateUpdatedAt:
            eligibilitySource.eligibilityStateUpdatedAt,
        }
