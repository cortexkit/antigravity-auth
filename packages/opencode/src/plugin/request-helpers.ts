 ...
 * @param schema - The JSON Schema node to clean up
 ...
 * @param isClaudeModel - Whether the model is a Claude model (affects signature validation)
 */
...
export function normalizeThinkingConfig(config: unknown): ThinkingConfig | undefined {
    ...
    if (!enableThinking && !finalInclude && thinkingBudget === undefined && includeThoughts === undefined) {
        return undefined;
    }
...
export function extractUsageFromSsePayload(payload: string): AntigravityUsageMetadata | null {
    ...
    for (const line of lines) {
        ...
        try {
            ...
            // continue (loop naturally continues)
        }
    }
    return null;
}
...
/**
 * Create a proper HTTP 403 response for rate-limit / quota-exhausted situations.
 *
 * HTTP 403 (not 429) because OpenCode treats 429 as retryable (isRetryable=true
 * from SDK) and retries up to 3 times before emitting session.error, during
 * which Omo Slim's ForegroundFallbackManager never fires. 403 stops immediately
 * -> session.error emitted -> Omo Slim catches it and triggers model fallback.
 *
 * Body is plain text so OpenCode renders it as a readable error message
 * instead of dumping raw JSON. Text contains "rate limit" which matches
 * Omo Slim's isRateLimitError regex (/rate.?limit/i).
 */
export function createRateLimitErrorResponse(
    ...
    const headers: Record<string, string> = {
        ...
        "Cache-Control": "no-cache",
        "X-Antigravity-Synthetic": "true",
        "X-Antigravity-Error-Type": "rate_limit_all_accounts",
    };
